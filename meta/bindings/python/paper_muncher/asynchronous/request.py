...
async def consume_paper_muncher_request(
    stdout: StreamReader,
    timeout: int
) -> None:
    ...
    while line := await readline_with_timeout(
        stdout,
...
async def read_paper_muncher_request(
    stdout: StreamReader,
    timeout: int,
) -> Optional[str]:
    ...
    """
    deadline = time.monotonic() + timeout
    first_line_bytes = await readline_with_timeout(
        stdout,
        timeout=remaining_time(deadline)
    )
    ...
    await consume_paper_muncher_request(stdout, timeout=remaining_time(deadline))
