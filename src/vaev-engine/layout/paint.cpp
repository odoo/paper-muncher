...
static Math::Radiif _resolveRadii(Resolver& resolver, Math::Radii<Calc<Length, Percent>> const& baseRadii, RectAu const& referenceBox) {
    Math::Radiif radii;
    ...
    return clip.shape.unwrap().visit(
        ...
        [&](Circle const& circle) {
            ...
            if (circle.radius.is<Keywords::ClosestSide>()) {
                ...
                radius = resolver.resolve(
                                     circle.radius.unwrap<Calc<Length, Percent>>(),
        ...
        [&](Ellipse const& ellipse) {
            ...
            if (ellipse.rx.is<Keywords::ClosestSide>()) {
                ...
                rx = resolver.resolve(
                                 ellipse.rx.unwrap<Calc<Length, Percent>>(),
                                 referenceBox.width
                )
                         .cast<f64>();
            }
            ...
            if (ellipse.ry.is<Keywords::ClosestSide>()) {
                ...
                ry = resolver.resolve(
                                 ellipse.ry.unwrap<Calc<Length, Percent>>(),
                                 referenceBox.height
                )
                         .cast<f64>();
            }
