...
export struct TableFormatingContext : FormatingContext {
    ...
    void computeFixedColWidths(Tree& tree, Box& box, Au knownSizeX) {
        ...
        if (not(box.style->sizing->width.is<Keywords::Auto>() or box.style->sizing->width.is<Calc<Length, Percent>>()))
        ...
        for (auto& col : cols) {
            ...
            if (not(width.is<Keywords::Auto>() or width.is<Calc<Length, Percent>>()))
                logWarn("width can't be anything other than 'auto' or a length in a table context");

            if (auto widthCalc = width.is<Calc<Length, Percent>>()) {
        ...
        while (x < grid.size.x) {
            ...
            auto cellBoxWidthCalc = cell.box->style->sizing->width.is<Calc<Length, Percent>>();
    ...
    Pair<Au> getCellMinMaxAutoWidth(Tree& tree, Box& box, TableCell& cell, Au tableComputedWidth, UsedSpacings usedSpacings) {
        ...
        if (not(cell.box->style->sizing->width.is<Keywords::Auto>() or cell.box->style->sizing->width.is<Calc<Length, Percent>>()))
            logWarn("width can't be anything other than 'auto' or a length in a table context");

        if (auto cellBoxWidthCalc = cell.box->style->sizing->width.is<Calc<Length, Percent>>()) {
    ...
    void computeAutoWidthOfColGroups(Tree& tree, Vec<Au>& minColWidth, Au tableWidth) {
        for (auto& group : colGroups) {
            ...
            auto columnGroupWidthCalc = columnGroupWidth.is<Calc<Length, Percent>>();
    ...
    void computeAutoWidthOfCols(Tree& tree, Vec<Au>& minColWidth, Vec<Au>& maxColWidth, Au tableWidth) {
        for (auto& [start, end, el] : cols) {
            ...
            auto widthCalc = width.is<Calc<Length, Percent>>();
    ...
    void computeRowHeights(Tree& tree) {
        ...
        for (auto& [row, index] : iter(rows) | Index()) {
            ...
            auto heightCalc = height.is<Calc<Length, Percent>>();
        ...
        for (usize i = 0; i < grid.size.y; ++i) {
            for (usize j = 0; j < grid.size.x; ++j) {
                ...
                if (not(cell.box->style->sizing->height.is<Keywords::Auto>() or cell.box->style->sizing->height.is<Calc<Length, Percent>>()))
                    logWarn("height can't be anything other than 'auto' or a length in a table context");

                auto rowSpan = cell.box->style->table->rowSpan;
                if (auto cellBoxHeightCalc = cell.box->style->sizing->height.is<Calc<Length, Percent>>()) {
