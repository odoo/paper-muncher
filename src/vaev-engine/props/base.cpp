...
static auto debugProperties = Debug::Flag::debug("css-properties", "Print debug information about CSS properties", true);
    ...
    struct Registration : Meta::NoCopy {
        ...
        virtual void inherit(ComputedValues const& parent, ComputedValues& child) {
            ...
            if (flags().has(INHERITED))
                logWarn("property {#} marked as INHERITED is using the slow fallback path. override inherit()!", name());
