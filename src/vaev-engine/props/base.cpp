...
struct DeferredProperty : Property {
    ...
    Res<Rc<Property>> _expandProperty(SpecifiedValues& child) const {
        ...
        // Acordingly, we need to eat them before parsing
...
export struct PropertyRegistry {
    ...
    Res<Rc<Property>> parseDeclaration(Symbol propertyName, Str propertyValue, Flags<Options> options) {
    ...
    Opt<Rc<Property>> parsePresentationAttribute(Symbol attrName, Str value) const {
        auto registration = try$(_presentationAttributes.tryGet(attrName));
        Css::Lexer lex{value};
        auto [content, _] = Css::consumeDeclarationValue(lex);
        Cursor<Css::Sst> cursor = content;
        return registration->parse(cursor).ok();
    }
};
