...
export struct Media {
    ...
    static Media forView(Math::Vec2i viewport, ColorScheme colorScheme) {
        return {
            ...
            .aspectRatio = viewport.width / static_cast<f64>(viewport.height),
    ...
    static Media forPrint(Print::Settings const& settings) {
        return {
            .type = MediaType::SCREEN,
            .width = Au{settings.paper.width} / Au{settings.scale},
            .height = Au{settings.paper.height} / Au{settings.scale},
