...
struct CounterSet {
    ...
    static CounterSet inherits(CounterSet& parent, CounterSet& sibling) {
        ...
        // NOTE: This is handled by the caller, as we don't have access to the element here.
    ...
    Counter& instantiateCounter(Dom::ElementHandle el, CounterProps::Reset const& reset, Integer initial) {
        ...
        // 3. If innermost counter’s originating element is element or a previous
        ...
        // Append a new counter to counters with name name, originating element element,
    ...
    void set(Dom::ElementHandle el, CounterProps::Set const& set) {
...
struct CounterStyleSet {
    ...
    Vec<CounterSymbol> _dispatchCounterFallback(CounterStyle const& style, Integer value, Set<CustomIdent>& seen) {
        ...
        // 3. Using the counter value and the counter algorithm for the counter
        //    style, generate an initial representation for the counter value.
        //
        //    If the counter value is negative and the counter style uses a
        //    negative sign, instead generate an initial representation using
        //    the absolute value of the counter value.
        auto maybeRepr = _dispatchCounterSystem(style, Math::abs(value));
        if (maybeRepr == NONE)
            return _dispatchToFallbackCounter(style, value, seen);
