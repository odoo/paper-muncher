...
struct ValueParser<FitContent> {
    static Res<FitContent> parse(Cursor<Css::Sst>& c) {
        ...
        if (c->prefix == Css::Token::function("fit-content(")) {
            ...
            result.value = try$(parseValue<LengthPercentage>(scan));
            c.next();
            return Ok(result);
        }
...
export using Size = FlatUnion<Keywords::Auto, LengthPercentage, Keywords::MinContent, Keywords::MaxContent, FitContent>;
export using MaxSize = FlatUnion<Keywords::None, LengthPercentage, Keywords::MinContent, Keywords::MaxContent, FitContent>;
