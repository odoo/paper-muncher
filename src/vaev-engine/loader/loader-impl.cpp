...
Async::Task<Rc<Scene::Node>> _fetchImageContentAsync(Http::Client& client, Ref::Url url, Async::CancellationToken ct) {
    ...
    if (resp->header.contentType().unwrapOr(Ref::sniffBytes(data)).conformsTo(Ref::Uti::PUBLIC_SVG)) {
        ...
        auto width = resolver.resolve(root->computedValues()->sizing->width.unwrapOr<Calc<Length, Percent>>(Length{300_au}), 300_au);
        auto height = resolver.resolve(root->computedValues()->sizing->height.unwrapOr<Calc<Length, Percent>>(Length{300_au}), 300_au);
