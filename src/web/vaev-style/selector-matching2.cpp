...
static Selector _invertTree(Selector const& selector){
    Selector inverted;

    selector.visit(Visitor{
        [&](Nfix const& n) {
            // If the selector is a NFIX, we take it and invert it's parts.
            
        },[&](Meta::Contains<UniversalSelector,ClassSelector,IdSelector,TypeSelector> auto & s) {
            // If it's unitary (class, id, type, universal) we take it as is.
            
        },[&](Infix const& s) {
            // If the selector is a INFIX, we advise based on the operator.
            
            if (s.type == Infix::DESCENDANT) {
                // If the operator is DESCENDANT, invert it and it's parts.
                
            } else if (s.type == Infix::CHILD) {
                // If the operator is CHILD, we invert it and it's parts.
                
            } else if (s.type == Infix::ADJACENT) {
                // If the operator is ADJACENT, we take it and invert it's parts.
                
            } else if (s.type == Infix::SUBSEQUENT) {
                // If the operator is SUBSEQUENT, we take it and invert it's parts.
                
            } else {
                logWarnIf(DEBUG_MATCHING, "unimplemented infix invertor for type: {}", s.type);
            }

        },[&](Pseudo const& s) {
            // If the selector is a Pseudo, we take it and invert it's parts.
            
        },
        [&](auto const& s) {
            logWarnIf(DEBUG_MATCHING, "unimplemented invertor for selector: {}", s);
        }
    });
    return inverted;
}
