...
Res<None, Output> shouldAbortFragmentingBeforeLayout(FragmentationContext &fc, Input input) {
    if (not(fc.allowBreak() and fc.isDiscoveryMode))
        return Ok(NONE);
...
Output layout(Tree &tree, Box &box, Input input) {
    ...
    input.ancestralsBorderPadding = input.ancestralsBorderPadding + borders.bottomEnd() + padding.bottomEnd();
