...
static Rc<Scene::Clip> _resolveClip(Frag const& frag) {
    ...
    return clip.shape.unwrap().visit(Visitor{
        [&](Polygon const& polygon) {
            ...
            result.moveTo(referenceBox.xy.cast<f64>() + Math::Vec2f(resolver.resolve(it->v0, referenceBox.width).cast<f64>(), resolver.resolve(it->v1, referenceBox.height).cast<f64>()));
            for (auto& point : Slice(it + 1, end(polygon.points))) {
                result.lineTo(referenceBox.xy.cast<f64>() + Math::Vec2f(resolver.resolve(point.v0, referenceBox.width).cast<f64>(), resolver.resolve(point.v1, referenceBox.height).cast<f64>()));
            }
