...
static Text::ProseStyle _proseStyleFomStyle(Style::Computer& c, Style::Computed& style) {
...
void _flushRootInlineBoxIntoAnonymousBox(Box& parent, Rc<InlineBox>& rootInlineBox) {
    ...
    parent.add({style, parent.fontFace, std::move(*rootInlineBox), nullptr});
...
export struct BlockFlowBuilder {
    ...
    BlockFlowBuilder(Style::Computer& c, Box& box)
        : box(box), rootInlineBox(makeRc<InlineBox>(_proseStyleFomStyle(c, *box.style))) {}
    ...
    static void createBoxAndBuildfromElement(Style::Computer& c, Box& parent, Rc<Style::Computed> style, Gc::Ref<Dom::Element> el) {
        auto font = _lookupFontface(c.fontBook, *style);
