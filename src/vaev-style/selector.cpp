...
export struct Selector : _Selector {
    ...
    Selector() : _Selector{TypeSelector{NONE, NONE}} {}
    ...
    static Res<Selector> _parseSelectorElement(Cursor<Css::Sst>& cur, OpCode currentOp) {
        ...
        if (*cur == Css::Sst::TOKEN) {
            ...
            case Css::Token::IDENT:
                val = TypeSelector{NONE, Symbol::from(cur->token.data)};
                break;
            case Css::Token::DELIM:
                if (cur->token.data == ".") {
                    cur.next();
    ...
    static Res<Selector> parse(Cursor<Css::Sst>& c) {
        ...
        while (not c.ended()) {
            currentSelector = try$(_parseInfixExpr(currentSelector, c, OpCode::NOP));
        }
