...
Rectangle<PercentOr<Length>> buildRectangle(Style::SpecifiedStyle const& style) {
    return {
        style.svg->x,
        style.svg->y,
...
Circle<PercentOr<Length>> buildCircle(Style::SpecifiedStyle const& style) {
    return {
        style.svg->cx,
        style.svg->cy,
        style.svg->r,
...
Rc<Math::Path> buildPath(Style::SpecifiedStyle const& style) {
...
struct Shape {
    ...
    static Type _getTypeFromTag(TagName tagName) {
        if (tagName == Svg::PATH) {
            return Type::PATH;
    ...
    static Shape build(Rc<Style::SpecifiedStyle> style, TagName tagName) {
        return {
...
bool isShape(TagName tagName) {
    return tagName == Svg::PATH or tagName == Svg::CIRCLE or tagName == Svg::RECT;
}
...
Math::Trans2f computeEquivalentTransformOfSVGViewport(ViewBox const& vb, Vec2Au const& position, Vec2Au const& size) {
    ...
    return Math::Trans2f::scale({(f64)scaleX, (f64)scaleY}).translated({(f64)translateX, (f64)translateY});
}
